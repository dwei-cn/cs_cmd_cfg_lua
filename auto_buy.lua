local a = gui.Tab(gui.Reference("Visuals"), "improvments", "Autobuy and fixes")
...
local f = gui.Groupbox(a, "Settings", 15, 15, 300, 600)
local g = gui.Checkbox(f, "autobuy.active", "Enable Auto Buy", true)
local h = gui.Checkbox(f, "autobuy.printlogs", "Print Logs To Aimware Console", false)
local i = gui.Combobox(f, "autobuy.primary", "Primary Weapon", b[1][1], b[2][1], b[3][1], b[4][1], b[5][1],b[6][1])
local j = gui.Combobox(f, "autobuy.secondary", "Secondary Weapon", c[1][1], c[2][1], c[3][1], c[4][1],c[5][1])
...
-- 设置默认值
i:SetValue(5)  -- 默认AK 47 | M4A1
j:SetValue(0)  -- 默认Five Seven | Tec 9
k:SetValue(2)  -- 默认Kevlar Vest + Helmet
l:SetValue(2)  -- 默认Flash
m:SetValue(3)  -- 默认Smoke
--n:SetValue(0)  -- 默认Off
--o:SetValue(0)  -- 默认Off
--p:SetValue(true)  -- 默认买Taser
--q:SetValue(true)  -- 默认买Defuse Kit
h:SetValue(true)  -- 默认打印日志
